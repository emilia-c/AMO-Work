...
class MEP:
    ...
    def get_mep_data(self):
        try:
            ...
            # Scraping assistants from the first structure
            ...
            # Scraping assistants from the second structure
            assistants_grid = soup.find_all('div', class_='ep_gridrow-content')

            if assistants_grid:
                for section in assistants_grid:
                    assistant_type_tag = section.find('h3', class_='ep-a_heading ep-layout_level3 erpl-meps-asst-type-title')
                    if assistant_type_tag:
                        assistant_type = assistant_type_tag.find('span', class_='ep_name').get_text(strip=True)
                        if assistant_type not in self.assistants:
                            self.assistants[assistant_type] = []

                        assistant_tags = section.find_all('span', class_='ep_name')
                        for assistant_tag in assistant_tags:
                            assistant_name = assistant_tag.get_text(strip=True)
                            if assistant_name:
                                print(f"Saving assistant: {assistant_name}")  # Debugging line
...
def save_to_json(meps_list):
    json_filename = "29term_apas_w_nationalParty_NOMANUAL2.json"
    try:
