        ...
        try:
            ...
            for section in assistants_sections:
                ...
                if accredited_assistants_section:
                    # Dynamically find the assistant type from the surrounding header
                    assistant_type_tag = accredited_assistants_section.find_previous('h4', class_='erpl_title-h4')
                    if assistant_type_tag:
                        assistant_type = assistant_type_tag.get_text(strip=True)  # Get the text from the header, e.g. "Accredited assistants (grouping)"
                    else:
                        assistant_type = "Unknown assistant type"  # Fallback if no header is found
