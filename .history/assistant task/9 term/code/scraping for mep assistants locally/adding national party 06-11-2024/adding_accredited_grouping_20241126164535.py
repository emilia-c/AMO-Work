    ...
    def get_mep_data(self):
        try:
            ...
            for section in accredited_assistants_sections:
                # Dynamically get the assistant type from the heading (e.g., "Accredited assistants")
                assistant_type = section.get_text(strip=True)  
                
                # Initialize the assistant type in the dictionary if it doesn't exist
                if assistant_type not in self.assistants:
                    self.assistants[assistant_type] = []

                # Extract assistant names under this heading
                accredited_assistants_section = section.find_all('div', class_='erpl_type-assistants')
