        ...
        try:
            ...
            if assistants_sections:
                for section in assistants_sections:
                    ...
                    # Only process certain types of assistants
                    if assistant_type in ["Accredited assistants", "Accredited assistants (grouping)", "Local assistants", "Service providers", "Paying agents"]:
                        ...
                        name_tags = section.find_all('li')  # Assuming you meant find_all to extract multiple assistants
                        for li in name_tags:
            ...
            # Scrape Assistant Information in the second structure
            assistants_section = soup.find('div', class_='erpl_type-assistants')
            if assistants_section:
                for section in assistants_section:
                    # Extract the assistant type based on the header
                    ...
                    if assistant_type_tag:
                        ...
                        continue  # Skip if no header found for this section

                    # Initialize the assistant type if not already present
                    if assistant_type not in self.assistants:
                        self.assistants[assistant_type] = []
