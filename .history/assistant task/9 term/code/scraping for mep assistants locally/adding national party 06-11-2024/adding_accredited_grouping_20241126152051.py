    ...
    def get_mep_data(self):
        try:
            ...
            if accredited_assistants_section:
                assistant_type = "MUG"  # Assuming MUG is the assistant type
                if assistant_type not in self.assistants:
                    self.assistants[assistant_type] = []

                # Try to find assistant names in <span> tags first (as in the first structure)
                ...
                # If no names found in <span>, try finding them in <li> tags (fallback structure)
                if not self.assistants[assistant_type]:  # Only fallback if no assistants found yet
                    li_tags = accredited_assistants_section.find_all('li')
                    for li_tag in li_tags:
                        assistant_name_tag = li_tag.find('span', class_='ep_name')
                        if assistant_name_tag:
                            assistant_name = assistant_name_tag.get_text(strip=True)
                            if assistant_name and assistant_name not in self.assistants[assistant_type]:
                                self.assistants[assistant_type].append(assistant_name)
            ...
            # New method to find assistants in the "ep_gridrow ep-o_productlist" structure
            group_assistants_section = soup.find('div', class_='ep_gridrow ep-o_productlist erpl-meps-asst-group-list')
            if group_assistants_section:
                assistant_type = "TIDDLYWINK"
                if assistant_type not in self.assistants:
                    self.assistants[assistant_type] = []

                # Look for the <ul> element which contains the list of assistants
                ul_tag = group_assistants_section.find('ul')
                if ul_tag:
                        # Find all <li> tags within the <ul>
                    li_tags = ul_tag.find_all('li')
                    for li_tag in li_tags:
                            # For each <li>, find the <span class="ep_name"> that contains the assistant name
                        assistant_name_tag = li_tag.find('span', class_='ep_name')
                        if assistant_name_tag:
                            assistant_name = assistant_name_tag.get_text(strip=True)
                            if assistant_name and assistant_name not in self.assistants[assistant_type]:
