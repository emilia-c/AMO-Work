    ...
    def get_mep_data(self):
        try:
            ...
            for section in assistants_sections:
                ...
                # Find all sections with potential assistant types
                assistant_sections = soup.find_all('h4', class_='erpl_title-h4')

                for section in assistant_sections:
                    # Dynamically get the assistant type from the heading
                    assistant_type = section.get_text(strip=True)  # Extract the text of the heading
                    
                    # Skip if the assistant type is not in the valid list
                    ...
                    # Find the corresponding assistant container
                    assistants_container = section.find_next('div', class_='erpl_type-assistants')
                    if not assistants_container:
                        continue
                    
                    # Extract all assistant names from <span class="erpl_assistant">
                    assistant_tags = assistants_container.find_all('span', class_='erpl_assistant')
                    for assistant_tag in assistant_tags:
                        # Extract the text, ignoring any SVG or other nested tags
                        assistant_name = ''.join(assistant_tag.stripped_strings)  # Combines all text ignoring nested elements
