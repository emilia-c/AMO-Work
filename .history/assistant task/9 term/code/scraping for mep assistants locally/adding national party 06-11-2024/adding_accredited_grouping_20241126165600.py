...
class MEP:
    ...
    def get_mep_data(self):
        try:
            ...
            if not self.assistants:  # Only process if no assistants were already found
                accredited_assistants_section = soup.find('div', class_='erpl_type-assistants')
                if accredited_assistants_section:
                    # Try to find a nearby heading (e.g., <h4> or <h3>) for the assistant type
                    assistant_type_section = accredited_assistants_section.find_previous(['h4', 'h3'], class_='erpl_title-h4')
                    if assistant_type_section:
                        assistant_type = assistant_type_section.get_text(strip=True)  # Extract the assistant type dynamically
                    else:
                        assistant_type = "Unknown assistant type"  # Default if no heading is found
                    
                    # Ensure that the assistant type is added to the dictionary
                    if assistant_type not in self.assistants:
                        self.assistants[assistant_type] = []

                    # Extract assistant names from the section
...
def save_to_json(meps_list):
    json_filename = "term_apas_w_nationalParty_noMANUAL.json"
    try:
