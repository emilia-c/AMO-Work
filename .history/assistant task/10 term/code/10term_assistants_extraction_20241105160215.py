...
# Scrape all the MEP information wanted (MEP name, MEP party, MEP country of origin, assistant names, assistant type)
class MEP:
    ...
    def get_mep_data(self):
        ...
        if mep_party_tag:
            ...
            print(f"Warning: MEP Party information not found for {self.url}")
        
        # Scrape MEP country
        mep_country_tag = soup.find('div', class_='erpl_title-h3 mt-1 mb-1')
        if mep_country_tag:
...
def main():
    ...
    with open("mep_assistants.json", "w", encoding="utf-8") as outfile:
