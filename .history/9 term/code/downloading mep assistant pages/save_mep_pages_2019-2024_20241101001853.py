...
save_dir = "mep_assistant_pagesTEST"
...
async def fetch_snapshot(session, client, mep_id, mep_name, save_path):
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            archive_url = f"https://web.archive.org/web/{snapshot_date}/{url}"
            ...
            async with session.get(archive_url) as response:
                ...
                # Print the HTML content being downloaded (for debugging)
                print(f"Downloaded HTML for {mep_name}:\n{content[:500]}...")  # Print first 500 characters
                ...
                return archive_url  # Return the archive URL
    ...
    async with aiohttp.ClientSession() as session:
        ...
        with tqdm(total=total_meps, desc="Downloading MEPs", unit="MEP") as pbar:
            for mep in sorted_meps:
                ...
                archive_url = await task  # Wait for the fetch_snapshot to complete and get the archive URL

                # Update the progress bar with the MEP name and archive URL
                pbar.set_postfix({"Current MEP": mep_name, "Archive URL": archive_url})  # Show the correct archive URL
                pbar.update(1)
