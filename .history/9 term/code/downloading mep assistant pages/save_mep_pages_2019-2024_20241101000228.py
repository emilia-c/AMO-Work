    ...
    async with aiohttp.ClientSession() as session:
        ...
        with tqdm(total=total_meps, desc="Downloading MEPs", unit="MEP") as pbar:
            for mep in sorted_meps:
                ...
                # Update the progress bar with the MEP name and URL
                generated_url = base_url.format(mep_id, urllib.parse.quote(mep_name, safe="+"))
                pbar.set_postfix({"Current MEP": mep_name, "URL": generated_url})  # Print the correct URL here
                pbar.update(1)
